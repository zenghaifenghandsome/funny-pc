...
const TimeAgo = (props:any) =>{
    ...
    useEffect(()=>{
        const showTime = (time:any)=>{
           let showTime:string = time.slice(0,-1).split('T')[0]+" "+time.slice(0,-1).split('T')[1].substring(0,8)
            return showTime; // 2017-03-31 16:02:06
        }
        const rtime =(time:any)=>{

           let new_datetime = time.slice(0,-1).split('T')[0]+" "+time.slice(0,-1).split('T')[1].substring(0,8) // 2017-03-31 08:02:06

           var timestamp:any = new Date(Date.parse(new_datetime));
