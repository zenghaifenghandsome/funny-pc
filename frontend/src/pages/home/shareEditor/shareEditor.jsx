...
const ShareEditor = () => {
    ...
    const htmlEscape = (html) => {
      ...
      const getDataOfEditorMultiple = () => {
        return document.getElementById('editorMultiple')?.innerHTML || '';
        //console.log('childrenData', childrenData)
        //return htmlEscape(childrenData||'');
        //console.log('toServiceData', toServiceData)
    }
    const upload = async (uploadProps) =>{
        if(picList.length<9){
    ...
    const delete_pic = (pic) => {
        setPicList(picList.filter(p => p != pic));
    }
    const send = async () => {
        let share = {
            userid: userinfo.userid,
            content: getDataOfEditorMultiple(),
            piclist: picList.join()
        }
        console.log(share)
        let result= await api_addShare(share);
        if(result.status === 200){
            Message.success(result.msg)
            nav("/")
        }else{
    ...
    const addTopic = async () => {
        ...
        console.log(topic)
        let result = await api_addTopic(topic)
        console.log(result)
        if(result.status === 200){
            Message.success(result.msg)
        }else{
